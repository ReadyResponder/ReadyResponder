class Person < ActiveRecord::Base
  ...
  attr_accessible :firstname, :lastname, :status, :icsid, :city, :state, :zipcode, :start_date, :title, :gender, :division1, :division2, :certs_attributes
  ...
  def age
    if @person.date_of_birth.present?
      Date.today.year - @person.date_of_birth.year + ( @person.date_of_birth.yday < Date.today.yday ? 1 : 0 )
    end
  end
